...
function renderWatchlist() {
  ...
  if (watchlistGrouped) {
    ...
    allAssets.forEach(({ asset, cat }) => {
      ...
      // Flat-view card: price + change on the right, category badge
      // moves inline with asset name so it doesn't compete with the price.
      const _fpd = (typeof priceData !== 'undefined') ? priceData[asset.id] : null;
      const _fPrice  = _fpd?.price;
      const _fChange = parseFloat(_fpd?.change || 0);
      const _fPriceText  = (_fPrice != null && isFinite(_fPrice))
        ? formatPrice(_fPrice, asset.id) : '\u2014';
      const _fChangeText = (_fPrice != null && _fpd?.change != null && _fpd.change !== '0.0000')
        ? ((_fChange >= 0 ? '+' : '') + _fChange.toFixed(2) + '%') : '';
      const _fChangeClass = _fChange > 0 ? 'pos' : _fChange < 0 ? 'neg' : '';
      card.innerHTML =
        '<div class="asset-left">' +
          '<div class="asset-symbol">' + asset.symbol + '</div>' +
          '<div class="asset-name">' + asset.name + ' <span class="wl-cat-badge">' + catLabel + '</span></div>' +
        '</div>' +
        '<div class="asset-price-wrap" style="margin-left:auto;flex-shrink:0;align-items:flex-end">' +
          '<div class="asset-price" id="wl-price-' + asset.id + '">' + _fPriceText + '</div>' +
          '<div class="asset-change ' + _fChangeClass + '" id="wl-change-' + asset.id + '">' + _fChangeText + '</div>' +
        '</div>';
      card.onclick = (e) => {
        if (e.target.classList.contains('wl-swipe-delete')) return;
        if (_swipeActive === card) { _swipeReset(card); return; }
        if (_swipeActive) { _swipeReset(_swipeActive); return; }
        navigateToChartOnSelect = true;
        selectAsset(asset);
      ...
      // Wrap in swipe-row so slide-left reveals the delete button,
      // matching the grouped view behaviour.
      const _fRow = document.createElement('div');
      _fRow.className = 'swipe-row';
      _fRow.style.marginBottom = '8px';
      card.style.marginBottom = '0';

      const _fDel = document.createElement('button');
      _fDel.className = 'swipe-delete';
      _fDel.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none"><polyline points="3 6 5 6 21 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M10 11v6M14 11v6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
      _fDel.onclick = (e) => {
        e.stopPropagation();
        _doRemoveFromWatchlist(asset.id, asset.symbol);
      };

      _fRow.appendChild(_fDel);
      _fRow.appendChild(card);
      _wireSwipeRow(_fRow, card);
      flat.appendChild(_fRow);
      totalCount++;
    });
  }
