...
async function fetchStrengthPrices() {
  ...
  // Fetch daily candles for the 28 pairs — gives us reliable open/close
  // Use granularity:86400 count:1 to get today's open and current close in one go
  // This is more reliable than tick history which can return empty on quiet pairs
  const pairs = Object.keys(CS_DERIV_SYM);
  const pendingSyms = new Set(Object.values(CS_DERIV_SYM));

  return new Promise(resolve => {
    let resolved = false;
    const doResolve = () => { if (!resolved) { resolved = true; clearTimeout(timeout); resolve(); } };
    const timeout = setTimeout(() => {
      console.warn('[Strength] Fetch timeout. Loaded:', Object.keys(_csPrices).length, '/', 28);
      doResolve();
    }, 15000);
    ...
    try {
      ...
      ws.onopen = () => {
        // Send a candle request per pair — granularity 3600 (1h), count 2
        // This reliably returns open/close data even for less-active pairs
        pairs.forEach((pairId, i) => {
          // Stagger by 20ms to avoid overwhelming the WS connection
          setTimeout(() => {
            if (ws.readyState !== WebSocket.OPEN) return;
            ws.send(JSON.stringify({
              ...
              count:             2,       // last 2 candles (prev + current)
              end:               'latest',
              adjust_start_time: 1,
            }));
          }, i * 20);
      ...
      ws.onmessage = (evt) => {
        try {
          ...
          // Extract from candles (preferred) or ticks (fallback)
          let openPx = null, closePx = null;
          if (msg.candles && msg.candles.length > 0) {
            const first = msg.candles[0];
            const last  = msg.candles[msg.candles.length - 1];
            openPx  = parseFloat(first.open);
            closePx = parseFloat(last.close);
          ...
          if (openPx && closePx) {
            ...
            // Market closed or error — still mark as handled
            pendingSyms.delete(sym);
          }

          if (pendingSyms.size === 0) {
      ...
      ws.onerror = (e) => {
        console.warn('[Strength] WS error:', e);
        doResolve();
      };
      ws.onclose = () => {
        // If we close with pending symbols, still resolve with what we have
        if (pendingSyms.size > 0) {
          console.warn('[Strength] WS closed with', pendingSyms.size, 'pairs pending');
        }
        doResolve();
      };
    } catch(e) {
      console.warn('[Strength] Fetch error:', e);
      doResolve();
    }
  });
}
